...
def _client() -> MagicMock:
    ...
    client.get_webinar_details.return_value = ZoomSessionDetails(
...
class TestUnresolvableTargets:
    ...
    def test_one_broken_target_does_not_cost_the_others_their_retry(self) -> None:
        ...
        client.get_meeting_transcript.side_effect = [
            RuntimeError("boom"),
            ZoomTranscript(download_url="https://zoom.example/transcript.vtt"),
        ]
...
class TestPermissionParity:
    ...
    def _populate_access(self, client: MagicMock) -> None:
        ...
        client.list_past_meeting_participants.return_value = [
            ZoomParticipant(user_email="attendee@example.com")
        ]
        client.list_meeting_registrants.side_effect = lambda session_id: _for_session(
            session_id,
            [ZoomRegistrant(email="registrant@example.com", status="approved")],
        )
        client.list_meeting_invitees.side_effect = lambda session_id: _for_session(
            session_id, [ZoomInvitee(email="invitee@example.com")]
        )
        client.list_past_webinar_participants.return_value = [
            ZoomParticipant(user_email="viewer@example.com")
        ]
        client.list_webinar_registrants.side_effect = lambda session_id: _for_session(
            session_id, [ZoomRegistrant(email="signup@example.com", status="approved")]
        )
        client.list_webinar_panelists.side_effect = lambda session_id: _for_session(
            session_id, [ZoomPanelist(email="panelist@example.com")]
        )

    def _crawled(self, client: MagicMock, work: OccurrenceWork) -> Document:
        items = list(process_occurrence(client, work, include_access=True))
        doc = items[0]
        assert isinstance(doc, Document)
        return doc
